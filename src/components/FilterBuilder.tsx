    ...
    ({ onFiltersChange, className, moves = [] }) => {
        ...
        const getAvailableConditions = (fieldId: string): FilterOperator[] => {
            ...
            return allOperators.filter(
                (op) =>
                    op.appliesTo.includes(field.type) &&
                    (!allowed || allowed.has(op.id)),
            );
        };
        ...
        useEffect(() => {
            const currentActiveFiltersString =
                JSON.stringify(currentActiveFilters);

            if (
                currentActiveFiltersString !== previousActiveFiltersRef.current
            ) {
            ...
            (id: string) => {
                setFilters((currentFilters) => {
                    if (currentFilters.length === 1) {
                        const desired = "input";
                        const hasDesired = gameConfig.fields.some(
                            (f) => f.id === desired,
                        );
                        const fallback = "impact";
                        ...
                        return [
                            {
                                ...
                                field: firstField,
                                numericField:
                                    fType === "number" ? firstField : null,
                                condition:
                                    getAvailableConditions(firstField)[0]?.id ??
                                    "equals",
                                value: "",
                                value2: "",
                            },
                        ...
                        prevFilters.map((filter) => {
                            if (filter.id === id) {
                                ...
                                if (property === "field") {
                                    ...
                                    const oldFieldType = getFieldType(
                                        filter.field,
                                    );
                                    ...
                                    const available =
                                        getAvailableConditions(value);
                                    if (
                                        !available.some(
                                            (c) => c.id === filter.condition,
                                        )
                                    ) {
                                        updatedFilter.condition =
                                            available[0]?.id || "equals";
                                    }
                ...
                <div className="p-4 border rounded-md bg-card space-y-4">
                    ...
                    <div className="space-y-2">
                        {filters.map((filter) => {
                            ...
                            const showRange = isRangeCondition(
                                filter.condition,
                            );
...
const FilterRow = React.memo<FilterRowProps>(
    ({
        ...
        const multi =
            availableConditions.find((op) => op.id === filter.condition)
                ?.input === "multi";
        return (
            <div
            ...
            >
                <Combobox
                    value={filter.field}
                    onChange={(val) =>
                        val && updateFilter(filter.id, "field", val)
                    }
