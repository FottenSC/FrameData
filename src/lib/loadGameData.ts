...
const parseAvailableColumns = (source: unknown, gameId: string): ColumnId[] => {
  if (!Array.isArray(source)) {
    throw new Error(`${gameId}: availableColumns must be an array`);
  }
  const columns: ColumnId[] = [];
  const seen = new Set<ColumnId>();
  for (const value of source) {
    if (!isColumnId(value)) {
      throw new Error(`${gameId}: unknown column ${String(value)}`);
    }
    if (seen.has(value)) {
      throw new Error(`${gameId}: duplicate column ${value}`);
    }
    seen.add(value);
    columns.push(value);
  }
