...
export const CommandRenderer: React.FC<{ command: string | null }> = ({ command }) => {
  ...
  for (let i = 0; i < commandArray.length; i++) {
    ...
    for (let b of buttons.split("+")) {
      ...
      if (button === "_") {
        ...
        parts.push(<img key={`direction-${i}-${buttonIndex}-${button}`} src={iconUrl} alt={button} className="inline object-contain align-text-bottom h-4 w-4" />);
      } else {
