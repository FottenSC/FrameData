...
import { prefetchGameData } from "@/lib/loadGameData";
...
export const GameSelectionPage: React.FC = () => {
  ...
  //
  // Character thumbnails are left to native `loading="lazy"` on the next
  // page; prefetching the whole roster here would compete with navigation.
  useEffect(() => {
  ...
  // on one tile before clicking. Both calls are idempotent: a
            ...
            >
              <CardHeader className="grow">
                <h2 className="font-semibold leading-none tracking-tight flex items-center justify-center text-xl">
                  {game.icon || (
                    <Gamepad2 className="h-5 w-5 mr-2 opacity-70" />
                  )}
                  {game.name}
                </h2>
              </CardHeader>
