...
export const TableConfigurator: React.FC = () => {
  ...
  const onToggleVisible = (id: string, value: boolean) => {
    setColumnConfigs(prev => prev.map(c => (c.id === id ? { ...c, visible: value } : c)));
  };

  const modifiers = [restrictToVerticalAxis, restrictToParentElement];

  return (
  <DndContext sensors={sensors} onDragEnd={onDragEnd} modifiers={modifiers} collisionDetection={closestCenter}>
      <div className="px-2 py-2 select-none">
        <div className="mb-2 flex items-center justify-between">
          <div className="text-sm font-medium">Columns</div>
          <Button variant="outline" size="sm" className="h-7 text-xs" onClick={restoreDefaults}>
            Restore defaults
          </Button>
        </div>
        <SortableContext items={ids} strategy={verticalListSortingStrategy}>
          <div className="max-h-80 overflow-auto rounded border p-2">
            {ids.map((id) => {
              ...
              return (
                <Row key={id} id={id} label={conf.label} checked={conf.visible} onCheckedChange={(v) => onToggleVisible(id, v)} />
              );
            })}
          </div>
        </SortableContext>
...
const Row: React.FC<{
  ...
  return (
    <div ref={setNodeRef} style={style} className={cn("flex items-center gap-3 p-2 rounded border bg-background/70 transition-colors mb-2", isDragging && "ring-2 ring-primary/50")}> 
